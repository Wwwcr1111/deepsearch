...
_SEARCH_TOOLS: dict[str, SearchTool] = {}


def _get_search_tool(backend: str) -> SearchTool:
    """Create search backends lazily to avoid import-time side effects."""

    tool = _SEARCH_TOOLS.get(backend)
    if tool is None:
...
def dispatch_search(
    ...
    search_tool = _get_search_tool(search_api)
